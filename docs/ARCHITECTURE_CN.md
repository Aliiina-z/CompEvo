...
BASE_LLM_MODEL = "meta-llama/Llama-3.1-8B"  # Llama 3.1 8B (论文)
USE_QUANTIZATION = True  # 4-bit量化
MAX_LENGTH = 4096  # 上下文窗口 (论文)
NUM_AGENTS = 10  # 智能体数量 (论文)
NUM_EPOCHS = 3  # 训练轮数
```
...
```python
# 模型配置
BASE_LLM_MODEL = "meta-llama/Llama-3.1-8B"  # 从Llama-2-7B升级
USE_QUANTIZATION = True
MAX_LENGTH = 4096  # 上下文窗口
LORA_RANK = 16
LORA_ALPHA = 32
LORA_TARGET_MODULES = [  # 所有线性投影层
    "q_proj", "k_proj", "v_proj", "o_proj",
    "gate_proj", "up_proj", "down_proj"
]

# 训练配置
NUM_AGENTS = 10  # 从5增加到10
NUM_EPOCHS = 3
BATCH_SIZE = 2
LEARNING_RATE = 1e-4  # 从2e-5更新
TEMPERATURE = 0.5  # 论文最优值

# 损失权重 (已优化)
LAMBDA_STRATEGY = 1.0
LAMBDA_DIVERSITY = 0.4  # 最优sweet spot
LAMBDA_PRUNING = 0.1

# Token限制
MAX_NEWS_TOKENS = 2048  # 新闻部分限制
```
...
- `'GPU_LLAMA'`: 使用Llama-3.1-8B + 4bit量化
